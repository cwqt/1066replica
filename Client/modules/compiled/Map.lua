...
Map.addObject = function(x, y, obj)
  if Map.current[y][x].object == nil then
    ...
    obj.x, obj.y = x, y
    return log.info("Inserted " .. tostring(obj.__class.__name) .. "(" .. tostring(obj.isPlayer) .. ") at " .. tostring(x) .. ", " .. tostring(y))
  else
    return log.error("Object already exists at " .. tostring(x) .. ", " .. tostring(y))
  end
end
...
Map.moveObject = function(start, finish)
  ...
  if Map.current[toy][tox].object then
    log.error("moveObject: Object exists at " .. tostring(tox) .. ", " .. tostring(toy))
    return 
  end
...
Map.removeObjects = function()
  if #Map.deleteStack >= 1 then
    ...
    for _index_0 = 1, #_list_0 do
      ...
      log.debug("Removed object " .. tostring(obj))
